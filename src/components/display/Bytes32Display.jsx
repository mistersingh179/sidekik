...
export default function Bytes32Display(props) {
  const { value } = props;
  const [displayOption, updateDisplayOption] = useState("bytes32");

  let formattedValue;

  if (displayOption === OPTS.bytes32) {
    formattedValue = value.toString();
  } else if (displayOption === OPTS.string) {
    try {
      formattedValue = ethers.utils.parseBytes32String(value);
    } catch (e) {
      console.error("error parsing bytes32: ", value, e);
      formattedValue = "";
    }
